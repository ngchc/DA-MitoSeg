...
parser.add_argument('-sp', '--save-path', type=str, default='models_mk95')
parser.add_argument('-cp', '--cache-path', type=str, default='caches_mk95')
parser.add_argument('-pc', '--percent', type=int, default=95) # for mask
...
parser.add_argument('-ti', '--total-iters', type=int, default=10000)
parser.add_argument('-di', '--decay-iters', type=int, default=10000)
...
def load_dataset():
	...
	t1 = time()
	train_provider = Provider('../mitoem/mito_human_train', batch_size=opt.batch_size,
	                          num_workers=opt.num_process, percent=opt.percent)
	valid = Valid('../mitoem/mito_human_valid')
...
def build_model():
	...
	model = RSUNet([16,32,48,64,80]).to(device)
